...
exports.checkForUpdates = () => {
  if (
    ...
    return exports
      .request("https://api.github.com/repos/manuelvgx/ytdl-core/contents/package.json", {
        requestOptions: {
          headers: {
            "User-Agent":
  ...
  if (oldLocalAddressWarning) {
    ...
    console.warn(
      "\x1b[33mWARNING:\x1B[0m Using old localAddress option, " +
        "please add it to the agent options instead.",
    );
  }
};
    ...
    if (oldIpRotationsWarning) {
      ...
      console.warn(
        "\x1b[33mWARNING:\x1B[0m IPv6Block option is deprecated, " +
          "please create your own ip rotation instead.",
      );
    }
  }
};
...
exports.applyPlayerClients = options => {
  if (!options.playerClients || options.playerClients.length === 0) {
    options.playerClients = ["WEB_EMBEDDED", "IOS", "ANDROID", "TV", "WEB"];
  }
};
