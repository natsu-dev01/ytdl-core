...
//
// Old-format regex patterns (legacy YouTube player)
//
...
//
// New-format regex patterns (current YouTube player)
//
const USE_STRICT_Z_REGEXP = /'use strict';var\s+z\s*=\s*"([^"]*)"\s*\.split\s*\(\s*";"\s*\)\s*,/;
const PM_FUNCTION_REGEXP = /pM\s*=\s*function\s*\(([^)]*)\)\s*\{/;
const IM_OBJECT_REGEXP = /var\s+IM\s*=\s*\{/;
...
//
// New-format extraction functions (current YouTube player with pM/IM/z)
//

const extractZArray = (body) => {
  const match = body.match(USE_STRICT_Z_REGEXP);
  if (!match) return null;
  const values = match[1].split(";");
  return values;
};

const extractIMObject = (body) => {
  const startMatch = body.match(IM_OBJECT_REGEXP);
  ...
  const imStr = body.substring(startIdx, i);
  const methods = {};
  const methodRegex = /([a-zA-Z0-9_$]+):function\s*\(([^)]*)\)\s*\{([^}]*)\}/g;
  let m;
  while ((m = methodRegex.exec(imStr)) !== null) {
    methods[m[1]] = { params: m[2], body: m[3] };
  }
  return methods;
};

const extractPmFunction = (body) => {
  const startMatch = body.match(PM_FUNCTION_REGEXP);
  if (!startMatch) return null;
...
const resolveValue = (expr, I) => {
  if (typeof expr !== 'string') return expr;
  const trimmed = expr.trim();
  if (trimmed.includes('^')) {
    const parts = trimmed.split('^');
    const xorVal = parseInt(parts[parts.length - 1]);
    if (!isNaN(xorVal)) return I ^ xorVal;
  }
  return parseInt(trimmed);
};

const resolvePMOperations = (pMBody, zArray) => {
  const I = 1196;

  const opRegex = /IM\[z\[([A-Za-z0-9_$^]+)\]\]\(t,([A-Za-z0-9_$^]+)\);/g;
  const operations = [];
  let match;

  while ((match = opRegex.exec(pMBody)) !== null) {
    const keyExpr = match[1].replace(/\s+/g, '');
    const argExpr = match[2].replace(/\s+/g, '');

    const zIndex = resolveValue(keyExpr, I);
    if (zIndex === undefined || zIndex === null || isNaN(zIndex)) continue;
    if (zIndex < 0 || zIndex >= zArray.length) continue;

    const methodName = zArray[zIndex];
    const arg = resolveValue(argExpr, I);

    if (methodName === 'cE') {
      operations.push({ type: 'reverse' });
    } else if (methodName === 'xV') {
      operations.push({ type: 'splice', count: arg });
    } else if (methodName === 'PU') {
      operations.push({ type: 'swap', index: arg });
    }
  }
...
const buildNewDecipherFunc = (pMBody, zArray) => {
  const operations = resolvePMOperations(pMBody, zArray);
  if (!operations || operations.length === 0) return null;

  let body = "var " + DECIPHER_FUNC_NAME + "=function(" + DECIPHER_ARGUMENT + "){";
  body += "var t=" + DECIPHER_ARGUMENT + ".split('');";

  for (const op of operations) {
    if (op.type === 'reverse') {
      body += "t.reverse();";
    } else if (op.type === 'splice') {
      body += "t.splice(0," + op.count + ");";
    } else if (op.type === 'swap') {
      body += "var v=t[0];t[0]=t[" + op.index + "%t.length];t[" + op.index + "%t.length]=v;";
    }
  }

  body += "return t.join('');";
  body += "};";
  body += DECIPHER_FUNC_NAME + "(" + DECIPHER_ARGUMENT + ");";
...
const extractTceIfNeeded = (body) => {
  const tce = extractTceFunc(body);
  const name = tce?.name, code = tce?.code;
  return [extractDecipher(body, name, code), extractNTransform(body, name, code)];
};

const buildNewNTransformFunc = (pMBody, zArray) => {
  const operations = resolvePMOperations(pMBody, zArray);
  if (!operations || operations.length === 0) return null;

  let body = "var " + N_TRANSFORM_FUNC_NAME + "=function(" + N_ARGUMENT + "){";
  body += "var t=" + N_ARGUMENT + ".split('');";
  ...
  body += "return t.join('');";
  body += "};";
  body += N_TRANSFORM_FUNC_NAME + "(" + N_ARGUMENT + ");";
...
const extractNewFormatFunctions = (body) => {
  try {
    const zArray = extractZArray(body);
    if (!zArray) return null;

    const imObject = extractIMObject(body);
    if (!imObject) return null;

    const pMBody = extractPmFunction(body);
    if (!pMBody) return null;

    const decipherFunc = buildNewDecipherFunc(pMBody, zArray);
    const nTransformFunc = buildNewNTransformFunc(pMBody, zArray);

    if (!decipherFunc || !nTransformFunc) return null;

    const decipherScript = new vm.Script(decipherFunc);
    const nTransformScript = new vm.Script(nTransformFunc);
...
const extractDecipher = (body, name, code) => {
  return getExtractFunction([extractDecipherFunc], body, name, code);
};

const extractNTransform = (body, name, code) => {
  return getExtractFunction([extractNTransformFunc], body, name, code);
};

exports.extractFunctions = body => {
  const newResult = extractNewFormatFunctions(body);
...
exports.setDownloadURL = (format, decipherScript, nTransformScript) => {
  ...
  try {
    ...
    if (format.url) {
      ...
      if (!u.searchParams.has('alr')) u.searchParams.set('alr', 'yes');
